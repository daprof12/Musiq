 ...
 *   payment.failed     → log the failure (audit trail)
 *
 * Setup:
 ...
 *   https://qlfmuwefiehbctqipwpl.supabase.co/functions/v1/netreward-webhook
 */
...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers': 'content-type, x-client-info, apikey, x-nrt-signature',
...
async function handlePaymentCompleted(
  ...
  // Look up internal user by nrt_user_id stored in their profile
  const { data: profile, error: profileErr } = await supabase
    .from('profiles')
    .select('id')
    .eq('id', nrtUserId)   // Musiq uses auth UID as nrt_user_id
    .maybeSingle();
  ...
  // If it was a subscription, upgrade the user tier in profiles
  if (itemType === 'subscription') {
    const tier = (d.plan as string) || 'premium';
    ...
    if (tierErr) {
      ...
      console.log(`[NRT Webhook] User ${profile.id} upgraded to tier: ${tier}`);
    }
  }
}
  ...
  try {
    ...
    switch (event.type) {
      ...
      case 'payment.success':
        // NetReward also emits payment.success before final settlement
        await handlePaymentCompleted(supabase, event);
        break;
      default:
