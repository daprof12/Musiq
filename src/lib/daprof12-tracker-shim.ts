 ...
 *
 * Drop-in replacement for the @daprof12/tracker npm package.
 * The real package is hosted on GitHub Package Registry and requires a PAT.
 *
 ...
 *   3. In nrtTracker.ts, change the import from this shim to '@daprof12/tracker'
 *   4. Delete this file.
 *
 * This shim mirrors the exact public API of packages/tracker/src.
 */
...
class EventQueue {
  ...
  constructor(
    flushIntervalMs: number,
    maxBatchSize: number,
    onFlush: (events: TrackingEvent[]) => Promise<void>,
  ) {
...
export class NetRewardTracker {
  ...
  public startSimulation(gbPerHour = 1.0, tickIntervalMs = 5_000): void {
    ...
    this.simInterval = setInterval(() => {
      if (!this.currentSession) return;
      const upFraction = Math.random() * 0.4 + 0.1;
      this.reportUsage(
